...
if target_enabled "npm"; then
  ...
  NPMRC_BODY="proxy=$HTTP_PROXY
https-proxy=$HTTPS_PROXY
noproxy=$NO_PROXY
${CERT_LINE}
strict-ssl=true"
  write_user_block "$REAL_HOME/.npmrc" "$NPMRC_BODY"
  success "~/.npmrc 적용 (사용자: $REAL_USER)"

  # /etc/npmrc — 시스템 전역 npmrc. 'sudo npm install -g X' 시 root 가 읽음.
  # ~/.npmrc(사용자) 만 있으면 sudo npm 은 /root/.npmrc 를 읽어 프록시 못 찾고 실패.
  # 모든 사용자(root 포함)가 공유하므로 sudo npm install -g 가 정상 동작.
  ETC_NPMRC="/etc/npmrc"
  {
    echo "# >>> proxy-setup-system-begin <<<"
    echo "# Auto-generated by setup_proxy.sh — system-wide npm proxy"
    echo "$NPMRC_BODY"
    echo "# >>> proxy-setup-system-end <<<"
  } > "$ETC_NPMRC"
  chmod 644 "$ETC_NPMRC"
  success "/etc/npmrc 적용 (sudo npm install -g 도 동작)"
